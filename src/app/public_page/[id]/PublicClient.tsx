                ...
                <nav className="container mx-auto flex flex-wrap items-center justify-center px-4 py-3">
                    {["about", ...paintings.map((_, i) => `painting-${i}`), ...(projects.length ? ["projects"] : [])].map((tab) => {
                        ...
                        return (
                            <button
                                key={tab}
                                onClick={() => setSel(tab as typeof selected)}
                                className={`
            mx-2 my-1 px-4 py-2 text-white font-medium transition
            ${selected === tab ? "border-b-2 border-white" : "opacity-75 hover:opacity-100"}
          `}
                            >
                                {label}
                            </button>
            ...
            <main className="flex-grow container mx-auto px-6 py-10 space-y-16">
                ...
                {selected === "about" && (
                    <section className="space-y-8">
                        ...
                        {data.about.split("\n\n").map((para, i) => (
                            <p key={i} className="prose prose-lg mx-auto text-gray-700">
                                {para}
                            </p>
                        ))}
