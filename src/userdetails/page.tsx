...
import {
  FaSignOutAlt, FaCog, FaTimes, FaSave, FaUser, FaPhone, FaEnvelope, FaPlus, FaTrash
} from "react-icons/fa";
...
export default function UserDetails() {
  ...
  const [userDetails, setUserDetails] = useState({
    ...
    phone: "",
    email: "",
    paintings: Array(8).fill({ title: "", content: "" }), // Array di 8 quadri vuoti
  });
    ...
    fetch("/api/userDetails", {
      ...
      .then(data => {
        setUserDetails({
          ...
          phone: data.phone || "",
          email: data.email || "",
          paintings: data.paintings.length ? data.paintings : Array(8).fill({ title: "", content: "" }),
        });
      })
      .catch(() => {
  ...
  const handleSaveDetails = () => {
    ...
    fetch("/api/userDetails", {
      ...
      .catch(() => {
        setModalMessage("Errore durante il salvataggio dei dati.");
      });
  };
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-indigo-500 to-purple-700 text-white">
      ...
      <header className="w-full p-6 flex justify-between items-center bg-white shadow-lg">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => { localStorage.removeItem("token"); router.push("/"); }} className="flex items-center gap-2 bg-red-500 text-white px-4 py-2 rounded-lg shadow hover:bg-red-600 transition-all">
            <FaSignOutAlt /> Logout
          </button>
        </div>
      </header>
      ...
      <main className="flex flex-col items-center justify-center flex-grow text-center px-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg text-gray-900 w-full max-w-2xl">
              ...
              <div key={index} className="p-4 border rounded shadow-md bg-gray-50">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    updatedPaintings[index].title = e.target.value;
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    const updatedPaintings = [...userDetails.paintings];
                    updatedPaintings[index].content = e.target.value;
                    setUserDetails({ ...userDetails, paintings: updatedPaintings });
                  }}
          ...
          <div className="flex justify-end gap-2 mt-4">
            <button onClick={handleSaveDetails} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
              <FaSave /> Salva Dati
            </button>
          </div>
        </div>
